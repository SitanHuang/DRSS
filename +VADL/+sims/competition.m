...
mainSys = VADL.config.getMainSys(struct( ...
  ...
  ... 'windModelLowSpeed', 11, ...
  ... 'windModelHighSpeed', 23, ...
...
% Override with Ian-Anil'ism
sectionMasses = [14.9 12.6 10.70];
if ~mainSys.configParams.disableJettison
  sectionMasses = [7.4 12.6 10.70];
end
...
plot(resultantStates.x .* uc.m_to_ft, resultantStates.y .* uc.m_to_ft, 'Color', '#db4437', 'LineWidth', 2);
title("Flight Track");
ylabel("Altitude [ft]")
xlabel("Drift [ft]");
grid on;
...
legend('45deg 7mph with the wind', '45deg 7mph against the wind')