...
mainSys = VADL.config.getMainSys(struct( ...
  ...
  'launchSiteTemp', 55, ...
  'launchSitePressurePa', 97000, ...
  'motorOverride', 'K1100', ...
  'launchRailEffectiveTravel', 16, ... % ft
  'windSpeed', [[0, 10]; [1100, 8]], ...
  'driftCalcWindSpeed', 10, ...
  'windModelLowSpeed', 5, ...
  'windModelHighSpeed', 11, ...
  'windModelTurbulence', 0.05, ...
  'windModelFreq', 0.5 ...
));
...
%   'windSpeed', [[0, 15]; [1100, 10]], ...
%   'driftCalcWindSpeed', 15, ...
%   'windModelLowSpeed', 5, ...
%   'windModelHighSpeed', 14, ...
%   'windModelTurbulence', 0.05, ...
%   'windModelFreq', 2 ...
% ));
...
axis equal;
title("Lebanon")
saveas(fig, "340_track_lebanon", "png");
close(fig)
