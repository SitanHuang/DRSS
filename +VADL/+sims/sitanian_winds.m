...
for i = 1:length(windSpeeds_mph)
    ...
    payloadDriftFromApogee = ceil(abs(payloadStates.x(end) - apogeeState.x) * m_to_ft);
    % Payload drift from pad
    payloadDriftFromPad = ceil(abs(payloadStates.x(end)) * m_to_ft);
    % Rocket drift from apogee
    rocketDriftFromApogee = ceil(abs(resultantStates.x(end) - apogeeState.x) * m_to_ft);
    % Rocket drift from pad
    rocketDriftFromPad = ceil(abs(resultantStates.x(end)) * m_to_ft);
    % Rocket descent time
    rocketDescentTime = ceil(resultantStates.t(end) - t_apogee);
...
print(fig, "360_SitanianWinds.png", '-dpng', '-r600');
close(fig);
